...
​		想要正确地使用条件队列，关键是找出对象在哪个条件谓词上等待。条件谓词将在等待与通知等过程中导致许多困惑，因为在API中没有对条件谓词进行实例化的方法，并且在Java语言规范或JVM实现中也没有任何信息可以确保正确地使用它们。事实上，在Java语言规范或Javadoc中根本就没有直接提到它。但如果没有条件谓词，条件等待机制将无法发挥作用。

​		条件谓词是使某个操作成为状态依赖操作的前提条件。在有界缓存中，只有当缓存不为空时，`take`方法才能执行，否则必须等待。对`take`方法来说，它的条件谓词就是“缓存不为空”，`take`方法在执行之前必须首先测试该条件谓词。同样，`put`方法的条件谓词是“缓存不满”。条件谓词是由类中各个状态变量构成的表达式。
...
`ConditionObject`是同步器`AbstractQueuedSynchronizer`的内部类，因为`Condition`的操作依赖于相关联的锁。每个`Condition`对象包含着一个FIFO队列。`Condition`对象持有着首节点的引用`firestWaiter`和尾节点的引用`lastWaiter`。当前调用`Condition.await()`方法后，会以当前线程构造一个节点，并将节点加入到等待队列的尾部。该过程并没有CAS保证，因为该过程已经获得过锁。不再需要加锁。随后会（完全）释放锁，然后让线程阻塞直到被唤醒或被中断。如果线程从等待中唤醒，重新去获取锁。然后如果在阻塞中被中断，抛出中断异常。代码如下：
...
​		调用`Condition.signal()`方法，将会唤醒在等待队列中等待时间最长的节点。在唤醒之前，会将该节点移到同步队列中，前提是该线程获得锁。

​		移到同步队列后，当前线程再调用`LockSupport.unpark(Thread thread)`方法唤醒该节点中的线程。

​		被唤醒的线程循环中退出（`while(!isOnSyncQueue(Node node))`），进而调用`acquireQueued(node, savedState)`加入同步状态的竞争中。

​		成功获取同步状态后（即获得锁），被唤醒的线程从`condition.await()`方法中返回，此时，该线程已经获得锁。
