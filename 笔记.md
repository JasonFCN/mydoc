...
```java
//1. GET请求：http://localhost:8080/api/test?ids=1,2,3或者http://localhost:8080/api/test?ids=1&ids=2&ids=3    
	@GetMapping
    ...
    /**
     * 2. POST请求：http://localhost:8080/api/test
...
- 动态代理
  ...
  public class JDKProxy {
      ...
      public static void main(String[] args) {
          ...
          Hello proxyInstance = (Hello)jdkProxy.getProxyInstance();
          proxyInstance.hello();
      }
  }
  ...
  public class HelloWorldProxy implements MethodInterceptor {
      ...
      public static void main(String[] args) {
          ...
          System.out.println("代理对象调用方法 开始：");
          String hello = helloWorld.hello();
          System.out.println("代理对象返回结果：" + hello);
      }
  }
...
![线程池任务提交流程图](%E7%AC%94%E8%AE%B0.assets/%E7%BA%BF%E7%A8%8B%E6%B1%A0%E4%BB%BB%E5%8A%A1%E6%8F%90%E4%BA%A4%E6%B5%81%E7%A8%8B%E5%9B%BE.jpg)
...
| 类型                 | 核心线程数 | 最大线程数        | 超时时间 | 队列                | 线程工厂             | 拒绝策略    |
| -------------------- | ---------- | ----------------- | -------- | ------------------- | -------------------- | ----------- |
| FixedThreadPool      | n          | n                 | 0ms      | LinkedBlockingQueue | DefaultThreadFactory | AbortPolicy |
| SingleThreadExecutor | 1          | 1                 | 0ms      | LinkedBlockingQueue | DefaultThreadFactory | AbortPolicy |
| CachedThreadPool     | 0          | Integer.MAX_VALUE | 60s      | SynchronousQueue    | DefaultThreadFactory | AbortPolicy |
| ScheduledThreadPool  | n          | Integer.MAX_VALUE | 0ns      | DelayedWorkQueue    | DefaultThreadFactory | AbortPolicy |

#### 31，maven 源码安装到私有仓库

```cmd
clean source:jar install -DskipTests
```
...
server {
		...
		location  / {      
            proxy_pass  http://10.6.12.2:3100;  #前端地址      
		    
        }
		location  ~ .*\.(gif|jpg|jpeg|png)$ {
			expires 24h;  
			root F:/download/images/;#指定图片存放路径  
			proxy_store on;  
			proxy_store_access user:rw group:rw all:rw;  
			proxy_temp_path     F:/download/images/;#图片访问路径  
			proxy_redirect     off;  
...
```
http://localhost:3100/product-image/F2A7F2EDF3A1345C18F16B590A736237A.jpg
```
...
String s = StringUtils.arrayToDelimitedString(strings, ",");
```
...
- 引入依赖
    ...
    <!--jjwt-->
    		...
    		<!--如果您使用的是JDK 10或更早版本，并且还希望使用RSASSA-PSS（PS256，PS384，PS512）算法-->
            <dependency>
...
起初，密码以明文形式存储。会受`SQL注入`等攻击方式泄露密码。

单向哈希运算后存储。即使泄露了存储库，也只是公开了散列值，通过散列值推算密文比较困难。但是可以采用`彩虹表`攻击方式，也能获取到原密码。

哈希 + 随机盐运算后存储。有效地避免了彩虹表攻击。

自适应单向函数来存储密码。随着现代硬件的发展，哈希运算的成本越来越低。哈希存储方式不再安全。

自适应单向函数游戏占用资源（CPU，内存等）

#### 36，Oauth2中，授权模式中的授权码模式，为什么要通过一个中间code？

1. 验证客户端，客户端通过code，去请求授权服务器，可以验证客户端身份
2. 不让访问令牌暴露给浏览器或者资源拥有者。
